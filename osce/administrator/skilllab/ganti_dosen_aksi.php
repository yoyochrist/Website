<?php
	...
	if(mysqli_num_rows($resultsession) > 0)
	{
		while($rowsession = mysqli_fetch_array($resultsession))
		{
			...
			$sqlupdategrade = "UPDATE `grade` SET `lecturer_id` = '".$lecturer."' WHERE `student_id` = '".$rowsession['student_id']."' AND `station_id` = '".$rowroom[0]."' AND `exam_type` = 'SL' AND `session_id` = '".$session."' AND `global_scale` IS NULL";
