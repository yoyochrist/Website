...
$sql="CREATE TEMPORARY TABLE IF NOT EXISTS temp_table AS (
		SELECT a.student_id,d.name,a.session_id,sum(a.score*a.weight)/sum(3*a.weight)*100 as nilai,a.global_scale,a.comment
		FROM grade a 
...
$pdf->CELL(15,6,'NILAI','LTR',0,'C',1);
$pdf->CELL(35,6,'GLOBAL RATING','LTR',0,'C',1);
...
foreach($result as $qry){
	...
	$pdf->setXY(10,$y);
	$pdf->CELL(10,6,$no,1,0,'C',1);
	$pdf->CELL(24,6,$qry['student_id'],1,0,'C',1);
	...
	$pdf->CELL(15,6,round($qry['nilai'],2),1,0,'C',1);
	$pdf->CELL(35,6,strtoupper(getGlobalScaleName($qry['global_scale'])),1,0,'C',1);
...
if($qry['final_comment']){
	$pdf->setXY(10,$y);
	$pdf->setFont('Times','b',8);
	$pdf->multicell(0, 4, 'KOMENTAR AKHIR' , 0, 'L');
	$y = $y + 12;
	$pdf->setFont('Times','',9);
	$pdf->multicell(0, 4, $qry['final_comment'] , 0, 'L');
