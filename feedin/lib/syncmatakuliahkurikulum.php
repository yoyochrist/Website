<?php
	...
	$json_mk_recordset = file_get_contents('http://localhost/Feedin/lib/table.php?f=GetDictionary&table=mata_kuliah_kurikulum');
	...
	$id_mkRecord = ($ws->GetRecordSet('mata_kuliah_kurikulum',"","","",""));
	...
	else
	{
		...
		foreach($id_mkRecord['result'] as $array)
		{
			...
			//$record_kelas[] = $record;
			$run_kelas = $crud->insert_array("mata_kuliah_kurikulum",$record);
		}
