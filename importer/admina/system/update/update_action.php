...
$check_kode_pt = $db->fetch_single_row('config_user','id',1)->id_sp;

$check_count = file_get_contents('http://wildantea.com/feeder-free-up/count_version_left.php?local_last='.$check_latest_version->version.'&kode_pt='.$check_kode_pt);
...
if (count($dta_server_version)>0) {
    foreach ($dta_server_version as $version) {
          ...
          $data_update = file_get_contents('http://wildantea.com/feeder-free-up/update.php?version='.$version->version);
          ...
          $create = "";
            foreach ($data_update as $dt) {
              ...
              if ($dt->type_update=='data') {
                ...
                file_put_contents(SITE_ROOT."upload/".$dt->nama_file, fopen('http://wildantea.com/feeder-free-up/data/'.$version->version.'/'.$dt->nama_file, 'r'));
                ...
                $sukses++;
                   ...
                   $dbs = file_get_contents('http://wildantea.com/feeder-free-up/data/'.$version->version.'/'.$dt->nama_file);
                    $sql = '';
                    foreach (explode(";\n", $dbs) as $query) {
                        $sql = trim($query);
