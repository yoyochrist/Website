	...
	if ($jumlah>=500) {
		...
		if ($id_reg_pd!="" && $id_mk!="" && $id_kls!="") {
			...
			if ($check_krs['result']) {
				...
				++$error_count;
				$error_msg[] = "$error_nim $error_mk $error_kelas";
				$db->update('nilai',array('status_error' => 2, 'keterangan'=>"$error_nim $error_mk $error_kelas"),'id',$value->id_krs);
		}
