...
if (isset($_POST['semester'])) {
	...
	$query = $new_table->get_custom("select * from mat_kurikulum where kode_jurusan='".$_POST['jurusan']."' and tahun='".$_POST['tahun']."' $semester $status $jns_mk ",$columns);


} else {
	$query = $new_table->get_custom("select * from mat_kurikulum where kode_jurusan='".$_POST['jurusan']."' and tahun='".$_POST['tahun']."' ",$columns);
...
foreach ($query	as $value) {
	...
	if ($value->jns_mk=='S') {
		...
		$jns = 'Wajib Peminatan';
	} elseif ($value->jns_mk=='D') {
		$jns = 'Pilihan Peminatan';
	}
