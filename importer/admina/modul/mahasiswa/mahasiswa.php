...
switch ($path_act) {
  ...
  case "edit":
     $id_jur = $path_four;
    $data_edit = $db->fetch_custom("select * from mhs inner join mhs_pt on mhs.id=mhs_pt.id_mhs where mhs_pt.id=?",array("mhs.id" =>$path_id));
    foreach ($data_edit as $data_edit) {
      # code...
    }
        foreach ($db->fetch_all("sys_menu") as $isi) {
