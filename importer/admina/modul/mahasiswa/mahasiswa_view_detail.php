...
<div class="form-group">
          ...
          <?php foreach ($db->fetch_custom("select mulai_smt from mhs_pt inner join mhs on mhs_pt.id_mhs=mhs.id where mhs_pt.kode_jurusan='".$id_jur."' group by mulai_smt order by mulai_smt desc") as $isi) {
...
<div class="form-group">
       ...
       foreach ($db->fetch_all("sys_menu") as $isi) {
                      if ($path_url==$isi->url) {
                          if ($role_act["insert_act"]=="Y") {
                    ?>
          <a href="<?=base_index();?>mahasiswa/tambah/<?=$id_jur;?>" class="btn btn-primary btn-flat"><i class="fa fa-plus"></i> Tambah</a>
                          <?php
                          } 
                       } 
}
